...
def validate_CNN(model_dir_name, data):
    ...
    pyl.hist(fwhms, label = 'full test set', bins='auto', alpha=0.5, color='dodgerblue') 
    pyl.hist(fwhms_test_misclass, label = 'misclassed test set', bins='auto', alpha=0.3, color='darkviolet')#'purple') 
    pyl.xlabel('FWHM')
    pyl.ylabel('Count')
    pyl.legend(loc='best')
    ...
    pyl.hist(test_good_p, label='normalized confidence histogram', bins=bins, alpha=0.5, weights=weights*3, color='dodgerblue')#normed=True)#density=True)
