...
def validate_CNN(model_dir_name, data):
    ...
    base_model = keras.models.load_model(model_dir_name + 'models_lesslay16_256_lr=0.001_drop=0.2_split=0.2/' + "model_100")#'10epochs_basic_model/model_350')#file)
    ...
    unique_labs = int(len(np.unique(labels)))
    y_binary = keras.utils.np_utils.to_categorical(labels, unique_labs)
    X = np.asarray(X_valid).astype('float32')


    # SPLIT INTO FT and valid for rest
    # have fine tune as human vetted
    skf = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=0)
    print(skf)
    skf.split(cutouts, labels)

    for ft_index, valid_index in skf.split(cutouts, labels):
        X_ft, X_valid = X[ft_index], X[valid_index]
        y_ft, y_valid = y_binary[ft_index], y_binary[valid_index]
